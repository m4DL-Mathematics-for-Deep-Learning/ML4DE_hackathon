...
pred = torch.load("predictions.pt").cpu().numpy()
truth = torch.load("test_data.pt").cpu().numpy()
print(pred.shape)
print(truth.shape)
...
if pred.shape[1] > 5000:
    np.save("lorenz_prediction.npy", pred[0, 1:, :])
else:
    np.save("lorenz_prediction.npy", pred[0, :, :])
